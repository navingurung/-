...
function buildTrackerModel(status: Status): TrackerModel {
  ...
  const baseSteps: TrackerStep[] = [
    ...
    },
    {
      ...
      timeline: "通常 数日程度",
      icon: ClipboardList,
      variant: STATUS.trj_customer_registration_pending
        .variant as TrackerVariant,
      status: "pending",
    },
    {
      label: "処理中",
      timeline: "通常 3〜5営業日",
      icon: RefreshCw,
      variant: STATUS.trj_transaction_registration_pending
        .variant as TrackerVariant,
      status: "pending",
    },
    {
...
export default function RefundDetailClient({
  ...
  // 最新状態は onRefresh() 後の props から受け取るため、
  // ローカルで status / deletedAt は持たない
  ...
  const handleRefundCancel = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        `/refund_delete/dashboard/${refund.id}`,
        {},
        {
      ...
      // FastAPI の標準キー detail と、独自キー details の両方に対応する
      toast.error(
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          <CardContent className="space-y-4 pt-6">
            ...
            <Info
              label="生年月日"
              value={formatDateCompact(refund.birth_date)}
            />
            <Info
              label="在留資格"
              value={landStatusMap[refund.land_status] || "-"}
            />
          ...
          return (
            <div key={`${step.label}-${index}`} className="relative">
              ...
              {index < steps.length - 1 && (
                <div className="absolute left-[calc(50%+28px)] top-6 h-0.75 w-[calc(100%-56px)] overflow-hidden rounded-full bg-slate-200">
                  {isCompleted && (
                  ...
                  {isCurrent && step.animateLine && (
                    ...
                    >
                      <div
                        className={cn(
                          "tracker-flow absolute inset-y-0 w-1/2 rounded-full bg-linear-to-r from-transparent via-current to-transparent",
                          styles.flowText,
                        )}
                      />
                    </div>
            ...
            >
              <div
                className={cn(
                  ...
                  isCurrent && styles.currentCircle,
                  isTerminal && styles.currentCircle,
                  isCompleted && "border-slate-300 bg-slate-50 text-slate-400",
                  isPending && "border-slate-300 bg-white text-slate-400",
                )}
              >
                <Icon
...
function OutcomeCard({
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-3 rounded-xl border px-4 py-4 border-destructive/30 bg-destructive/5",
      )}
    >
      <div className={cn("mt-0.5", "text-destructive")}>
        {isRejected ? (
      ...
      <div className="space-y-1">
        <p className={cn("text-md font-semibold text-destructive")}>{title}</p>
